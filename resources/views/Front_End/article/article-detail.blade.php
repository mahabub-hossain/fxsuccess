        ...
        <div class="row">
          ...
          <div class="col-lg-12 forex-broker-info mt-1 mb-1-half">
            <div class="container-fluid text-dark-brown mb-1-half">
             ...
             <div class="col-md-12" id = "allcommentarticle">
                ...
                <?php
                $art_comments = DB::table('articlecomments')->select('email','id','likes_count','article_id','name','comment','created_at')->where('article_id','=',$article_by_id->id)->orderBy('id','desc')->get();
               // print_r($comments);
               //  exit;
             ?>
             ...
             @foreach ($art_comments as $item)
                <div class="mb-1  container-fluid reviewer">
                  <div class="row">
                    ...
                    <div class="col-md-10 col-12 ">
                      ...
                      <button  onclick="articleactOnChirp(event);" data-chirp-id="{{$item->id }}">Like</button>
                      <span  id="likes-count-{{ $item->id }}">{{$item->likes_count }}</span>
                      @if(Session::get('user_email') == $item->email)
                      <a href="javascript:void(0)" id="{{$item->id}} "data-toggle="modal" class="edit_art" style="border:1px solid #ddd;background:#03a9f4;padding:2px 8px;color:#f5f5f5;border-radius: 3px;" data-target="#myModalEditCom">Edit</a>
                     @endif
                    </div>
                  </div>
                </div>
              ...
              <script>
                ...
                var articleactOnChirp = function (event) {
                    var chirpId = event.target.dataset.chirpId;
                    var action = event.target.textContent;
                    toggleButtonText[action](event.target);
                    updateChirpStats[action](chirpId);
                    $.get('/articlelike/' + chirpId + '/act',
                        { action: action });
                };
                Echo.channel('article-events')
              ...
              <div class="modal-dialog modal-lg animated zoomIn animated-3x" role="document">
                <div class="modal-content modal1-bg">
                  ...
                  <div class="modal-body no-pb">
                      {!!Form::open(['url'=>'/updateartcmt','class'=>'review-form form-group field-radius is-empty no-m no-p','id'=>'articlecmt' ])!!}
                      {{method_field('PATCH')}}                      
                      <p class="form-group no-m no-p">
                          <input type="hidden" value="" name="art_cmt_id" class="art_cmt_id"/>
                         <input type="hidden" class="article_id" value="{{$article_by_id->id}}" name ="art_id"/>
                          <textarea rows="5" class="form-control form-custom edit-art-comment" name="comment" ></textarea>
                        </p>
          ...
          <script>
              $('.edit_art').on('click',function(){
                   ...
                   $.get('/articleeditcmt/'+cmtid,function(data){
                      $('#articlecmt').find('.edit-art-comment').val(data.comment);
                      $('#articlecmt').find('.art_cmt_id').val(data.id);
                   });
                });

                $('#articlecmt').on('submit',function(e){
                e.preventDefault();
                var data = $(this).serialize();
                var url = $(this).attr('action');
                $.post(url,data,function(data){
                  console.log(data);
                    $('#articlecmt').trigger('reset');
                    $('#allcommentarticle').load(location.href + ' #allcommentarticle');
