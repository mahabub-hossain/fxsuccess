                                   ...
                                   <div class="mb-1  container-fluid reviewer">
                                    <div class="row">
                                       ...
                                       <div class="col-md-10 col-12 ">
                                          ...
                                          @if(Session::get('user_email') == $review->email)
                                          <a href="" id="{{$review->id}}" class="edit_revcmt" style="border:1px solid #ddd;background:#03a9f4;padding:2px 8px;color:#f5f5f5;border-radius: 3px;" data-toggle="modal" data-target="#revmyModalEditCom">Edit</a>
                                          @endif
                 ...
                 alert('Please write your comment');
                      ...
                      var data = $(this).serialize();
                       ...
                       $.post(url,data,function(data){
                         ...
                         $('#reviewcomment').load(location.href + ' #reviewcomment');
...
@endsection
         <div class="modal" id="revmyModalEditCom" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
               ...
               <div class="modal-content modal1-bg">
                  <div class="modal-header modal-header-bg-signal">
                     ...
                     <div class="modal-body no-pb">
                           ...
                           {!!Form::open(['url'=>'/updatereview','class'=>'review-form form-group field-radius is-empty no-m no-p','id'=>'rev_cmt'])!!}
                           {{method_field('PATCH')}}
                           <p class="form-group no-m no-p unq">
                                 <input type="text" value="" name="rev_id" class="rev_id"/>
                              <input type="text" class="broker_id" value="{{$broker_by_id->id}}" name ="broker_id"/>
                                 <textarea rows="5" class="form-control form-custom edit-broker"  name="comment" ></textarea>
                           </p>
                     </div>
         ...
         <script>
            $('.edit_revcmt').on('click',function(){
                   var cmtid = $(this).attr('id');
                   //alert(cmtid);
                  // confirm("Press a button!");
                    $.get('/revieweditcmt/'+cmtid,function(data){
                      console.log(data);
                      $('.unq').find('.edit-broker').val(data.comment);
                      $('.unq').find('.rev_id').val(data.id);
                   });
                 });
                 $('#rev_cmt').on('submit',function(e){
                e.preventDefault();
                var data = $(this).serialize();
                var url = $(this).attr('action');
                $.post(url,data,function(data){
                    $('#rev_cmt').trigger('reset');
                    $('#reviewcomment').load(location.href + ' #reviewcomment');
                });
             });
            </script>
